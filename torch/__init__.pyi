...
class Tensor:
    ...
    def __getitem__(self, indices : Union[int, slice, Tuple[Union[slice, int], ...]]) \
        -> Any: ...
    @overload
    def __add__(self, other : Union[float, 'FloatTensor']) -> 'FloatTensor' : ...
    @overload
    ...
    # This typing of __eq__ violates the Liskov substitution principle, but
    # it's how PyTorch behaves at runtime, so :shrug:.
    @overload
    def __eq__(self, other: 'Tensor') -> 'BoolTensor': ... # type: ignore[misc]
    @overload
...
class FloatTensor(Tensor):
    ...
    def to(self, device: str) -> 'FloatTensor' : ...
...
def cat(_tensors: Sequence[T], dim: int = 0) -> T: ...
...
def tensor(data: Union[bool, int, float, List[Union[bool, int, float]]], dtype: Any = float32) -> Tensor: ...

def log(data: FloatTensor) -> FloatTensor: ...
def abs(data: FloatTensor) -> FloatTensor: ...

def all(data: BoolTensor) -> bool: ...
def any(data: BoolTensor) -> bool: ...
